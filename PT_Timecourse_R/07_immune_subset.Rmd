...
library("dunn.test")
sobject_metadata = data.frame(sobject.sub.mac.wt@meta.data) #creates dataframe of metadata in sobject
sobject_metadata.sub = sobject_metadata[sobject_metadata$immunetype %in% c("DAMs", "Macrophage_Ldlrad3"),]
sobject_metadata.sub2 = sobject_metadata.sub[sobject_metadata.sub$timepoint %in% c("7dpi", "21dpi"),]
sobject_metadata.sub2$time_type = paste0(sobject_metadata.sub2$timepoint, "_", sobject_metadata.sub2$immunetype)
dunn.test(x = sobject_metadata.sub2$fab5_score1, g = sobject_metadata.sub2$time_type, method="bonferroni", kw=T, label=T)
#alt sig
sobject_metadata = data.frame(sobject.sub.mac.wt@meta.data) #creates dataframe of metadata in sobject
