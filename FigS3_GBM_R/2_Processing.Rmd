...
```
```{r}
#get markers for mouse clusters
#mouse genes --> human homologs
#jaccard similarity analysis

#get markers for mouse clusters

#use fibroblast reclustered, WT only spreadsheet
mouse_marker_file = "/Users/nathanewing-crystal/Dropbox (Ari Molofsky Lab)/Ari Molofsky Lab Team Folder/Nathan Ewing-Crystal/RNASeq/Lesion snRNASeq/seurat/Spreadsheets/lesion_snRNASeq_fib_reclustered_all_markers_wt.csv"
mouse_markers = read.csv(mouse_marker_file,stringsAsFactors = F)

#Select only the genes that pass thresholds
mouse_markers = mouse_markers[mouse_markers$p_val_adj<pval,]

#pick only positives, or restrict by min/max pct expression using pct1/2
mouse_markers = mouse_markers[mouse_markers$avg_log2FC > lfc,] #make log2FC >0.2 for heatmaps, >0 for volcano plots
mouse_markers = mouse_markers[mouse_markers$pct.1 > minpct & mouse_markers$pct.2 < maxpct,]

mouse_marker_genes = mouse_markers[,c("gene", "cluster")]
...
genes_human = getLDS(attributes = c("mgi_symbol"), filters = "mgi_symbol", values = mouse_marker_genes$gene, mart = mouse, attributesL = c("hgnc_symbol","hgnc_id",'ensembl_gene_id'), martL = human, uniqueRows=T)

print(head(genes_human))

mouse_marker_genes$human = rep(NA, length(mouse_marker_genes$gene))
for(gene in unique(mouse_marker_genes$gene)){
  index_add = which(mouse_marker_genes$gene == gene)
  index_lookup = which(genes_human$MGI.symbol == gene)
  if(length(index_lookup)>0){
    #if multiple options for human gene, take longer one
    if(length(index_lookup)>1){
      index_lookup = index_lookup[which.max(nchar(genes_human[index_lookup, "HGNC.symbol"]))]
    }
    add = genes_human$HGNC.symbol[index_lookup]
    mouse_marker_genes$human[index_add] = add
  }
}

mouse_marker_genes_v2 = mouse_marker_genes[,c("human","cluster")]
table(is.na(mouse_marker_genes$human)) #755 N/A, ~12.5%
mouse_marker_genes_v3 = mouse_marker_genes_v2[!is.na(mouse_marker_genes_v2$human),]
split_markers_mouse = split(mouse_marker_genes_v3, f = mouse_marker_genes_v3$cluster)
...
marker_genes = markers[,c("gene", "cluster")]
split_markers = split(marker_genes, f = marker_genes$cluster)

#jaccard similarity analysis

mutual_genes = intersect(marker_genes$gene,mouse_marker_genes_v3$human)

#### heatmap with jaccard dist. between n NEC and m Krummel factors
corr = matrix(0,length(split_markers_mouse),length(split_markers))
alpha = matrix(0,length(split_markers_mouse),length(split_markers))
row.names(corr) = names(split_markers_mouse[c(1:length(split_markers_mouse))])
colnames(corr) =  paste('GBM Cluster ',c(1:length(split_markers) - 1),sep='')
for(i in 1:length(split_markers_mouse)){
  temp_mouse = split_markers_mouse[[i]]$human
  temp_mouse = temp_mouse[match(mutual_genes,temp_mouse)]
  temp_mouse = temp_mouse[!is.na(temp_mouse)]
  for(j in 1:length(split_markers)){
    temp_human = split_markers[[j]]$gene
    temp_human = temp_human[match(mutual_genes,temp_human)]
    temp_human = temp_human[!is.na(temp_human)]
    
    jaccard = length(intersect(temp_mouse,temp_human))/length(union(temp_mouse,temp_human))
...
df = data.frame(corr)
df2 = df[names(sort(apply(df, 1, max), decreasing=T)),names(sort(apply(df, 2, max), decreasing=T))]

Plotfolder = "Plots/Fibroblast Subset/NMF"
name = paste0(project, iterationname, paste0("_jaccard.eps"))
setEPS()
postscript(file.path(dir, Plotfolder, name), height=5)
Heatmap(df2, name = paste0("Jaccard Similarity Score"),col = inferno(20),cluster_rows = FALSE,cluster_columns = FALSE)
dev.off()
```
...
get markers for mouse clusters
mouse genes --> human homologs
module score per human cluster of each mouse cluster
```{r}
for(i in 1:length(split_markers_mouse)){
  print(paste0("adding module score: ", names(split_markers_mouse[i])))
  skip_to_next <- FALSE
  temp_mouse = split_markers_mouse[[i]]$human
  tryCatch(sobject.sub.fib <- AddModuleScore(
    sobject.sub.fib,
    list(temp_mouse),
    pool = NULL,
    nbin = 24,
    ctrl = 100,
    name = paste0(names(split_markers_mouse[i]), "_score"),
    seed = 1,
    search = TRUE
...
Plotfolder = "Plots/Fibroblast Subset/Modules"
for(i in 1:length(split_markers_mouse)){
  name = paste0(names(split_markers_mouse[i]), "_score1")
  setEPS()
  postscript(file.path(dir, Plotfolder, paste0(name, ".eps")))
  print(VlnPlot(sobject.sub.fib, name))
  dev.off()
}
```
