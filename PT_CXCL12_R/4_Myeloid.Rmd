...
p3 = VlnPlot(sobject.sub.myeloid, features = "IFNgModuleScore1", group.by = "microanatomy", split.by = "genotype", split.plot=T)
p3$data$split <- factor(x = p3$data$split, levels = c("WT", "cKO_Cxcl12"))
name = paste0(project, iterationname, "_IFNgModuleScore1_split_microanatomy.eps")
...
sobject_metadata.mod$microanatomy = as.factor(sobject_metadata.mod$microanatomy)

# Apply wilcox_test separately for each myeloid type
results_list <- lapply(split(sobject_metadata.mod, sobject_metadata.mod$microanatomy), function(subset_data) {
  test <- wilcox_test(IFNgModuleScore1 ~ genotype | microanatomy, data = subset_data)
  data.frame(
    Microanatomy = unique(subset_data$microanatomy),
