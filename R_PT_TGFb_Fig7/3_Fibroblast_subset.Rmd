...
dev.off()
```
***NOTE - WITH DOUBLETS, MOVED TO "OLD WITH DOUBLETS FOLDER***
```{r}
...
***REDO ALL PLOTTING WITHOUT DOUBLETS***
```{r}
Plotfolder = "Plots/Fibroblast Subset"
DefaultAssay(sobject.sub.fib) = "RNA"
...
write.csv(markers_all,file = file.path(dir,"Spreadsheets",paste0(project, "_", iterationname, "_all_markers.csv")))
```
#NOTE - did this with doublets too, moved to new folder
```{r}
Idents(sobject.sub.fib) = "fibtype"
...
fibtype_keep = fibtype_keep[!(fibtype_keep %in% c("Fibroblasts_Fib_1", "Fibroblasts_Fib_2"))]
...
for(group in groups){
  ...
  newlist=list(de)
  de = read.csv(file.path(dir,"Spreadsheets/WT_vs_cKO", defile),stringsAsFactors = F) #any   spreadsheet with gene symbols or other identifiers
  colnames(de)[1] = "Gene"
  ...
  highlight_fib = c("Col1a1", "Col1a2", "Col4a5", "Col4a6", "Col6a5", "Col6a6", "Col8a1", "Col8a2", "Col11a1", "Col11a2", "Col12a1", "Col16a1", "Col18a1", "Col19a1", "Col22a1", "Col23a1", "Col24a1", "Col25a1", "Col26a1", "Col27a1",  "Ghr", "Ptgds", "Lama1", "Il31", "Tnc", "Fbn2", "Fn1", "Cd80", "Cthrc1", "Lrrc15", "Pdgfa", "Lum", "Dcn", "Mki67")
  highlight_fib = highlight_fib[highlight_fib %in% c(colorkeysup, colorkeysdown)]
  highlight_immune = c("Ccl5", "Cxcl5", "Ccl7", "Cxcl9", "Ccl9", "C3", "H2-Eb1", "H2-Q7", "Saa3", "Ccl6", "Il31", "Cxcl1", "Ccl8", "Il7", "Ccl2", "Cxcl13", "Il6")
  ...
  name = paste0("Fibroblasts ", clustername, "WT vs. Col1a2 cKO")
...
for(group in groups){
  ...
  write.csv(markers,file = file.path(dir,"Spreadsheets/WT_vs_ADWA11",paste0(project, "_", iterationname, "_cluster-", group_name, "_ctrl-vs-adwa11_markers.csv")))
  ...
  newlist=list(de)
  de = read.csv(file.path(dir,"Spreadsheets/WT_vs_ADWA11", defile),stringsAsFactors = F) #any   spreadsheet with gene symbols or other identifiers
  colnames(de)[1] = "Gene"
  ...
  #manual label
  clustername = (if(group_name == "all"){} else{paste0("(cluster ", group_name, ") ")})
  name = paste0("Fibroblasts ", clustername, "Ctrl vs. ADWA11")
  range = max(max(fc$avg_log2FC), min(fc$avg_log2FC))
  if(is.infinite(range)){range = 150}
...
```{r}
Plotfolder = "Plots/figure"
markers = c("Col1a1", "Col1a2", "Col8a1", "Col11a1", "Col12a1", "Col16a1", "Col25a1", "Col27a1", "Fn1", "Lrrc15", "Tnc", "Lum", "Col6a5", "Col4a5", "Col19a1", "Col23a1", "Col26a1", "Col18a1", "Col22a1","Col24a1", "Cthrc1",  "Mki67")
markers_short = c("Col1a1", "Col1a2", "Col8a1", "Col11a1", "Col12a1", "Col16a1", "Fn1", "Lrrc15", "Tnc", "Lum", "Col19a1", "Col23a1", "Col18a1", "Col22a1", "Cthrc1",  "Mki67")
...
setEPS()
postscript(file.path(dir,Plotfolder, "fib-heatmap.eps"))
DoHeatmap(
    object = avgexp, 
    features = markers,
    size = 5,
...
setEPS()
postscript(file.path(dir,Plotfolder, "fib-heatmap_short.eps"), width = 4)
DoHeatmap(
    object = avgexp, 
...
#compare myofibroblasts and ex-myofibroblasts
de = FindMarkers(
    sobject.sub.fib,
    group.by = "fibtype",
    ident.1 = "Myofibroblast",
    ident.2 = "ex-Myofibroblast")

de_myo = de[de$avg_log2FC>0,]
de_ex = de[de$avg_log2FC<0,]

comparison_delist = list(de_myo, de_ex)

clusters_named = c("Myofibroblasts", "ex-Myofibroblasts")
...
```
```{r}
markers_short = c("Col1a1", "Col1a2", "Col8a1", "Col11a1", "Col12a1", "Col16a1", "Col19a1", "Col22a1", "Col23a1", "Col18a1", "Fn1", "Lrrc15", "Tnc", "Lum", "Cthrc1",  "Mki67")

Idents(sobject.sub.fib) = "fibtype"
compare = subset(sobject.sub.fib, subset = fibtype %in% c("Myofibroblast", "ex-Myofibroblast"))

avgexp <- AverageExpression(compare, return.seurat = TRUE, verbose = TRUE)
...
```{r}
#add sub.cluster to total sobject
...
sub.cluster = as.character(sobject.sub.fib$sub.cluster)
names(sub.cluster) = as.character(colnames(sobject.sub.fib))

for(cell in names(celltype)){
  if(cell %in% names(sub.cluster)){
      index_cell = which(names(celltype) == cell)
      index_sub = which(names(sub.cluster) == cell)
      celltype[index_cell] = paste0("Sub_", sub.cluster[index_sub])
  }
}

sobject$sub.cluster = celltype
