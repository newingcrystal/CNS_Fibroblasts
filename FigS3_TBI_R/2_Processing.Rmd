...
name = paste0(project,"_",iterationname)
genes = c("COL1A2", "CTHRC1", "ACTA2", "CXCL12", "FAP", "IL33", "FN1")
groups = c("timepoint", "condition", "sampleid", "seurat_clusters")
...
#dim plots for clustering
for(group in groups){
  PrintSeuratGraph(sobject.sub.fib, namecard = name,graphtype = "dim",group = group, feature = group)
}
...
ngenes = 50
 ...
 defile = paste0(project, "_", iterationname, "TBI_vs_rest.csv")
  
#Comparison being made (must match file names)
comparison = paste0(project, "_", iterationname, "TBI_vs_rest")
...
#manual label
name = paste0("Fibroblasts: TBI vs. Rest")
range = max(max(fc$avg_log2FC), min(fc$avg_log2FC))
if(is.infinite(range)){range = 150}

setEPS()
postscript(file.path(dir, Plotfolder, paste0("Volcano_rest_vs_tbi.eps")))
